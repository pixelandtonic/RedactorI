...
/**
 * Redactor I field type class
 */
class RedactorIFieldType extends BaseFieldType
{
	// Properties
	// =========================================================================
	...
	private function _includeFieldResources($configJs)
	{
		...
		craft()->templates->includeJsResource('redactori/js/RedactorIInput.js');
	...
	private function _maybeIncludeRedactorPlugin($configJs, $plugin, $includeCss)
	{
		if (preg_match('/([\'"])'.$plugin.'\1/', $configJs))
		{
			if ($includeCss)
			{
				craft()->templates->includeCssResource('redactori/lib/redactor/plugins/'.$plugin.'.css');
			}

			craft()->templates->includeJsResource('redactori/lib/redactor/plugins/'.$plugin.'.js');
		}
	}
	...
	{
		$path = 'redactori/lib/redactor/lang/'.$lang.'.js';
